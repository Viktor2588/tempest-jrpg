...
[ ] **Phase 41 – Kampf-Rewrite II: Verschlinger (Predator) + Momentum-Tempo**
- `{type:'devour'}`-Verb, gated über Bruch + niedrige LP + Debuffs (Erfolgschance steigt mit Schwächung); **gewichteter Skill-Qualitätswurf** (schwach/stark), optional Absorptions-Tiefe; **Momentum** (Press-Turn-leicht) als beschränkter CT-Schub bei Schwäche-Ausnutzung/Devour (Terminierung bleibt garantiert). Rimuru erhält die Unique-Skills `predator` + `great-sage`. Tests: Gating, deterministischer Qualitätswurf, Momentum-Grenzen.
