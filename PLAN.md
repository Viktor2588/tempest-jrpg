...
[x] **Phase 37 – Imagegen-Overworld-Tiles II: Canon-Regionen** *(abgeschlossen 2026-06-30, Worktree `worktree/tempest-phase-37-canon-region-tiles`)*
> **✅ Status (2026-06-30):** Dwargon, Jura-Schlachtfeld, Echsen-Sumpf und Glutgrotte besitzen jetzt eigene 128×128-WebP-Boden-/Wandtiles statt generischem Gras-/Wand-Fallback. `OVERWORLD_TILE_THEMES`, Preload-Wiring, Texturfilterung, `ASSETS.md`-Provenienz und Asset-Gates decken alle acht Dateien ab; Contact-Sheet visuell geprüft. Abnahme: `bun run typecheck`, `bun run test` (**251/251**), `bun run build` und gezielter `bun run test:e2e -- --grep "Föderations-Save reist nach Blumund"` (**2/2**, Desktop + 390×844) grün.

[x] **Phase 38 – Imagegen-Gegner-Cutouts II: Canon-Bosse & Triggerkämpfe** *(abgeschlossen 2026-06-30, Worktree `worktree/tempest-phase-38-canon-enemy-cutouts`)*
> **✅ Status (2026-06-30):** Ork-Soldat, Ork-General, Orc-Disaster Geld, Echsenkrieger, Gabiru, Maskierter Majin und Ifrit besitzen jetzt eigene transparente 512×512-WebP-Kampf-Cutouts statt Kingdom-Atlas-/Namensfallbacks. Das Mapping deckt zusätzlich `orc-grunt`→Ork-Soldat und `orc-lord`→Orc-Disaster ab; Preload-Wiring, Texturfilterung, `ASSETS.md`-Provenienz, Alpha-Ecken und Contact-Sheet wurden geprüft. Abnahme: `bun run typecheck`, `bun run test` (**253/253**), `bun run build` und gezielter `bun run test:e2e -- --grep "Canon-Triggergegner"` (**2/2**, Desktop + 390×844) grün.

[x] **Phase 39 – Imagegen-Storyportraits III: Canon- & Regions-NPCs** *(abgeschlossen 2026-06-30, Worktree `worktree/tempest-phase-39-canon-npc-portraits`)*
> **✅ Status (2026-06-30):** Moorhüterin Eir, Schreinwächter Kael, König Gazel Dwargo, Kaval/Eren/Gido, Treyni, Milim Nava und Souka besitzen jetzt eigene projektgenerierte 512×512-WebP-Dialogportraits statt portraitloser Dialoge. Sprecher-Mapping, prozedurale Fallback-Specs, Preload-Wiring und `ASSETS.md`-Provenienz sind ergänzt; die sieben finalen Portraits wurden als 512×512-WebP geprüft und visuell gesichtet. Abnahme: `bun run typecheck`, `bun run test` (**254/254**), `bun run build` und gezielter `bun run test:e2e -- --grep "Canon- und Regions-NPCs"` (**2/2**, Desktop + 390×844) grün.
