...
>
> **Slice 4 umgesetzt (2026-06-29):** Dialoge zeigen nun kurze, überspringbare Story-Moment-Panels für Rekrutierung, Tempest-Benennung, Sturmschwur/Pakt, Rast und Quest-/Kapitelabschluss inklusive Belohnungstext. Mehrfach-Rekrutierungen werden in einem Panel zusammengefasst; SFX folgt dem Moment-Typ.
>
> **Abnahme (Slice 4 lokal verifiziert):** `npx --yes bun@latest run typecheck`, `npx --yes bun@latest run test` (**189/189**), `npx --yes bun@latest run build` und `npx --yes bun@latest run test:e2e` (**10/10**) grün.
>
> **Slice 5 umgesetzt (2026-06-29):** Ranga-Schnellreise erhält ein kurzes, überspringbares Reiseoverlay mit Routennotiz/Gefahrenkontext. Bei „reduzierte Bewegung" bleibt Schnellreise sofort und ohne Overlay. Das Viewmodell (`buildRangaJourneyView`) ist headless getestet.
>
> **Abnahme (Slice 5 lokal verifiziert):** `npx --yes bun@latest run typecheck`, `npx --yes bun@latest run test` (**190/190**), `npx --yes bun@latest run build` und `npx --yes bun@latest run test:e2e` (**10/10**) grün.
>
> **Slice 6 umgesetzt (2026-06-29):** Adaptive Tutorial-Codexanker erweitert: Direwolf-Boss erklärt nun Schwächefenster/Teamleiste klarer, nach dem Flüsterhain schaltet sich ein Echo-Boss-/Magieeffekt-Tutorial frei, und Ranga-Schnellreise wird erst nach real entdecktem sicherem Reisepunkt erklärt.
>
> **Abnahme (Slice 6 lokal verifiziert):** `npx --yes bun@latest run typecheck`, `npx --yes bun@latest run test` (**191/191**), `npx --yes bun@latest run build` und `npx --yes bun@latest run test:e2e` (**10/10**) grün.
...
- **Rekrutierungs- und Benennungsszenen:** Gobtas Beitritt, Rangas Pakt und Tempests erste Benennung erhalten jeweils einen klaren visuellen und akustischen Moment statt nur Flags und Questtext. Erste überspringbare Dialog-Moment-Panels mit passendem SFX sind umgesetzt; offen bleiben spätere individuelle Animationssets/Assets.
...
- **Boss-Nachspiel:** Direwolf und namenloses Echo erhalten Abschlusssequenz, Belohnungsübersicht, freigeschaltete Fähigkeiten/Systeme und sichtbare Reaktionen im Hub. Quest-/Kapitelabschluss-Dialoge zeigen nun ein kurzes Nachspiel mit Belohnungstext; für das namenlose Echo existiert zusätzlich Summary-/Lager-Nachspiel. Offen bleiben eigene Kampfende-Sequenzen und ein separater Belohnungsbildschirm.
- **Ranga-Reisegefühl:** Schnellreise und Scout-Bericht sind technisch umgesetzt; kurze überspringbare Reiseüberblendung mit Routennotiz/Gefahrenkontext sowie „reduzierte Bewegung“ als sofortige Variante sind umgesetzt. Offen bleiben spätere echte Ranga-Bewegungs-/Landschaftsassets und zusätzliche optionale Entdeckungen.
- **Adaptive Tutorials:** Schwächen/Teamleiste beim Direwolf, Status-/Magieeffekte beim Echo und Scout-/Schnellreise erst bei ihrer tatsächlichen Freischaltung erklären. Direwolf-, Flüsterhain-, Echo-Boss- und sichere Ranga-Schnellreise-Tutorialanker sind umgesetzt; offen bleiben spätere HUD-Hervorhebungen direkt während der jeweiligen Kampf-/Reiseszene.
