...
## Kampf-Rewrite „Verschlingen & Analysieren" (Predator-Analyse + Zeitleiste) — Phasen 40–46

> **Vision (2026-06-30, Spielerwunsch — großer Kampf-Rewrite):** Der bisherige Kampf (Angriff / MP-Skill-Spam / Team-Leiste) wird durch eine kanonische, lesbare Verb-Kette ersetzt: **Analysieren** (verborgene Schwäche + nächsten Zug aufdecken) → **Brechen** (Schwäche treffen, Deckung aufreißen) → **Verschlingen** (gebrochenen Gegner devouren: Sofort-K. o. **+ dauerhaft eine seiner Fähigkeiten lernen**). Verschlinger (Predator) ist Rimurus **Startfähigkeit**; sein Arsenal wächst durch Verschlungenes und Skills wie Wasserklinge/Wasserstrahl. Dazu: **einzigartiges Kampfkonzept je Kämpfer** (Signatur-Leisten), **Team-Mix/Fusionsangriffe**, **Zeitkontrolle** (Züge verzögern/beschleunigen) und **Aussetz-Status** (Betäubung/Schlaf/Versteinerung/…). Devouren ist kein bloßer Knopfdruck: starke Gegner brauchen Debuffs + niedrige LP; die **Skill-Qualität ist ein gewichteter Wurf** (z. B. ~85 % schwach / ~10 % stark), optional mit Absorptions-Tiefe. Architektur bleibt rein, deterministisch, headless-testbar; jede Phase eigener Worktree, Tests zuerst. *(Phasen 33–39 sind für den parallel laufenden Asset-Track reserviert, daher startet der Kampf-Arc bei 40.)*

[x] **Phase 40 – Kampf-Rewrite I: Neue Zeitleiste (Analyse, Bruch, CT-Verzögern/-Beschleunigen, Aussetz-Status)** *(abgeschlossen 2026-06-30, Worktree `worktree/tempest-phase-40-zeitleiste`)*
> **✅ Status (2026-06-30):** Substrat des Rewrites steht — **rein additiv**, damit der gesamte Bestand grün bleibt. Neu in der reinen Engine (`battle.ts`/`battleView.ts`, Phaser-/DOM-frei, seedbar): **Analyse-Aktion „Großer Weiser"** (`{type:'analyze'}`) deckt Schwächen auf, erhöht die `analysisLevel` (0→2), liest den nächsten Gegnerzug (**Telegraph**) und verstärkt den Break-Druck auf analysierte Schwächen; **Zeitkontrolle** als `ctDelta` auf Skills (delay wirft Gegner zurück, hasten zieht Verbündete vor — neue Skills `temporal-snare`/`quicken`); **Aussetz-/Behinderungs-Status** `stun`/`sleep`/`freeze`/`paralyze`/`petrify`/`blind`/`silence`/`confuse`/`charm`/`weaken` mit Zugüberspringen in `advanceToNextActor` (Schlaf/Eis enden bei Schaden, Verstummen blockiert Fähigkeiten, Geschwächt/Geblendet senken Offensive). View liefert `analysisLevel`/`revealedWeaknesses`/`telegraphSkillId`. Abnahme: `bun run typecheck` sauber, `bun run test` **255/255** (7 neue Phase-40-Checks), `bun run build` im Budget.

[x] **Phase 41 – Kampf-Rewrite II: Verschlinger (Predator) + Momentum-Tempo** *(abgeschlossen 2026-06-30, Worktree `worktree/tempest-phase-41-predator`)*
> **✅ Status (2026-06-30):** Rimuru startet mit den Unique-Skills `predator` und `great-sage`; beide sind nicht direkt wirkbar, sondern gaten die Kampfverben `analyze` und `devour`. `devour` verschlingt nur verwundbare Gegner über Bruch, niedrige LP oder Debuffs, nutzt eine seedbare Erfolgschance mit gewichteter Skill-Mimik und hält dauerhaftes Lernen bewusst für Phase 42 zurück. Schwächentreffer und Devour geben einen begrenzten CT-Momentum-Schub, ohne die Terminierung zu gefährden. Abnahme: `bun run typecheck`, `bun run test` (**260/260**) und `bun run build` grün; kein Browser-Smoke nötig, da reine Battle-/Datenlogik.

[ ] **Phase 42 – Permanente Skill-Aneignung & Rimurus Skill-Mix (Wasserklinge, Wasserstrahl, Verschlungenes)**
- Devour hängt dauerhaft an `learnedSkillIds` an (Persistenz über `party.ts`/`save.ts`, Dublettenschutz); `enemies.ts` bekommt `devourSkillId`/`devourable`; Rimurus Mix mit Magicule-Kosten + Loadout-Cap im Menü; Codex-Einträge „Verschlungen"; Anbindung an `rimuru-predator-*`-Knoten. Tests: Save-Roundtrip, keine Dubletten, Loadout-Cap.

[ ] **Phase 43 – Einzigartige Kampfkonzepte je Kämpfer (Signatur-Leisten)**
- **Ein** Signatur-Framework (Leiste + Signatur-Aktion) auf `CharacterDefinition`, generisch in der Engine aufgelöst; je Held datengetrieben instanziert (Rimuru/Ranga/Shuna/Benimaru/Shion/Hakurou/Souei/Gobta/Rigurd/Kurobe), verdrahtet die Phase-40/41-Primitive (Souei→delay, Ranga→hasten, Shuna→Reinigung, Benimaru→Brand-Stacks, Hakurou→Konter, Gobta→Mini-Mimik). HUD je Kämpfer. Tests je Signatur.

[ ] **Phase 44 – Team-Mix: Kombo-/Fusionsangriffe**
- Kombo-Resolver: ≥2 verbundene Verbündete (`RELATIONSHIPS`) geben Momentum gemeinsam aus → Fusionsangriff + Element-Fusionstabelle; Remap der bestehenden `combatBonus`/`teamAttack` auf Kombo-Freischaltungen (kompatibel, kein Content-Bruch). Tests: Gating, Fusionselemente, Determinismus, Remap.

[ ] **Phase 45 – KI, Telegraph & Tempo-Politur**
- `autoBattle.ts`: Gegner-KI nutzt die neuen Verben (analyze→Schwäche ausnutzen→brechen, Party verzögern/betäuben, Devour-Futter meiden, CT-Kontrolle + Aussetz-Status); Telegraph steuert informiertes Block-/Konter-Timing. Tests: Terminierung/CT-Grenzen mit KI, neue Verben, Determinismus.

[ ] **Phase 46 – Inhalt, Balance & Kampfpräsentation**
- Schwäche-/Devour-/Telegraph-/Signatur-/Kombo-Tabellen über alle Gegner inkl. Canon-Bosse; Schwellen tunen. `BattleScene`: `?`/Schwäche-Icons, Telegraph, Devour-Finisher, Signatur-Leisten, Kombo-Flash, Status-Icons. Deutsche Tutorial-/Codex-Texte; `playthrough`/`qaGates` erweitern. Abnahme: volle Suite grün, Build im Budget, Browser-/Handy-Smoke.
