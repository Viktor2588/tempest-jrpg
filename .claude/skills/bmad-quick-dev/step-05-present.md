...
1. If version control is available and the tree is dirty, create a local commit. Start the message with the branch's Jira key (e.g. `feature/ECOM-4555-...` → `ECOM-4555`), followed by a Conventional Commits type: `ECOM-1234 <type>: <summary>`. The key MUST stay first — it drives Jira deploy tags. No ticket key in the branch → plain conventional message.
